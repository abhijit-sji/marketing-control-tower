...
const isSlugTaken = async (slug: string, currentId?: string) => {
...
const generateUniqueSlug = async (name: string, currentId?: string) => {
  const baseSlug = slugify(name);
  let slug = baseSlug;
  let counter = 2;

  while (await isSlugTaken(slug, currentId)) {
    ...
    switch (method) {
      case "GET":
        if (brandId || brandSlug) {
          // Get single brand with KPIs - query by id or slug
          ...
          const { data: brand, error: brandError } = await query.single();
          ...
          const formattedBrand = {
            ...
            owner_name: brand.owner
              ? `${brand.owner.first_name || ""} ${brand.owner.last_name || ""}`.trim()
              : "Unknown",
            co_owner_name: brand.co_owner
              ? `${brand.co_owner.first_name || ""} ${brand.co_owner.last_name || ""}`.trim()
              : null,
      ...
      case "POST": {
        ...
        const slug = await generateUniqueSlug(createData.name);
        ...
        const formattedNewBrand = {
          ...
          owner_name: newBrand.owner
            ? `${newBrand.owner.first_name || ""} ${newBrand.owner.last_name || ""}`.trim()
            : "Unknown",
          co_owner_name: newBrand.co_owner
            ? `${newBrand.co_owner.first_name || ""} ${newBrand.co_owner.last_name || ""}`.trim()
            : null,
          kpis: [],
        };
      ...
      case "PUT": {
        ...
        const slug = await generateUniqueSlug(updateData.name, brandId);
        ...
        const formattedUpdatedBrand = {
          ...updatedBrand,
          owner_name: updatedBrand.owner
            ? `${updatedBrand.owner.first_name || ""} ${updatedBrand.owner.last_name || ""}`.trim()
            : "Unknown",
          co_owner_name: updatedBrand.co_owner
            ? `${updatedBrand.co_owner.first_name || ""} ${updatedBrand.co_owner.last_name || ""}`.trim()
            : null,
