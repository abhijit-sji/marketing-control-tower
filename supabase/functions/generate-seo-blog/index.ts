...
} from '../_shared/integrations/pgvector.ts'
  ...
  try {
    ...
    while (!validation.valid && currentAttempt <= maxRepairAttempts) {
      ...
      if (repairResult.validation.valid) {
        ...
        blog = repairResult.blog
        validation = repairResult.validation
        console.log(`Repair improved validation. Errors reduced from ${validation.errors.length} to ${repairResult.validation.errors.length}`)
    ...
    if (!validation.valid) {
      console.log(`Final result after ${currentAttempt - 1} repair attempts: Still has ${validation.errors.length} validation errors`)
 ...
 */
async function collectBrandAnalytics(
  supabaseClient: any,
  brandId: string
): Promise<string> {
  try {
 ...
 * Collect Brand KPIs
 */
async function collectBrandKPIs(
  supabaseClient: any,
  brandId: string
): Promise<string> {
  try {
