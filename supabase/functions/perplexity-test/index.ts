...
serve(async (req) => {
  ...
  try {
    ...
    if (action === 'research') {
      ...
      if (!researchTopic) {
        return new Response(
          JSON.stringify({ ok: false, error: 'Topic or prompt is required for research' }),
...
Return your response in this exact JSON format:
{
  ...
  "suggested_headline": "A compelling headline for a LinkedIn post about this topic"
      ...
      const response = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            { role: 'user', content: `Research this topic for LinkedIn content creation: ${researchTopic}. Focus on recent developments, trending aspects, and actionable insights. Return valid JSON only.` }
          ],
      ...
      if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({ ok: false, error: 'Research failed', details: errorData }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
    ...
    if (action === 'scrape') {
      ...
      if (!url) {
        return new Response(
          JSON.stringify({ ok: false, error: 'URL is required for scraping' }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
  ...
  "content_date": "YYYY-MM-DD if available, or null"
      ...
      const response = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            { role: 'system', content: systemPrompt },
            { role: 'user', content: `Analyze and summarize the content from this URL for our knowledge base: ${url}. Return valid JSON only.` }
          ],
          temperature: 0.2,
      ...
      if (!response.ok) {
        ...
        return new Response(
          JSON.stringify({ ok: false, error: 'Scrape failed', details: errorData }),
          { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      const response = await fetch('https://api.perplexity.ai/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a helpful AI assistant that provides concise, accurate information about current trends and topics.'
            },
            {
              role: 'user',
              content: testPrompt
